...
def load_cicids_sample():
    ...
    logger.info(f"Loaded: {df.shape[0]} rows, {df.shape[1]} cols from {file_path}")
    return df
...
def preprocess(df):
    ...
    if y.nunique() < 2:
        ...
        y.iloc[:n] = 1 - y.iloc[:n]
        logger.warning(f"Single class detected; flipped {n} samples")
...
def run_anomaly_detection(X, y):
    ...
    logger.info(f"Isolation Forest accuracy: {accuracy:.2f}")
...
def run_supervised_detection(X, y):
    ...
    logger.info(classification_report(y_test, preds, target_names=["Attack", "Benign"]))
    logger.info(f"ROC AUC: {roc_auc_score(y_test, probs):.3f}")
