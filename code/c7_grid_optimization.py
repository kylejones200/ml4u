...
class VoltageControlEnv(gym.Env if HAS_GYM else object):
    ...
    def step(self, action):
        ...
        return (
            np.array([self.voltage, self.load], dtype=np.float32),
            reward, False, False, {}
        )
    ...
    for ep in range(episodes):
        ...
        for _ in range(steps):
            ...
            q_table[v_bin, l_bin, action] += alpha * (
                reward + gamma * np.max(q_table[nv_bin, nl_bin]) -
                q_table[v_bin, l_bin, action]
            )
            v_bin, l_bin = nv_bin, nl_bin
            total_reward += reward
        if ep % print_interval == 0:
            logger.debug(f"Episode {ep}: Reward {total_reward:.2f}")
