...
def load_feeder_map():
    ...
    if not os.path.exists(shapefile):
        print(f"Feeder shapefile not found at '{shapefile}'. Proceeding without base map.")
        return None
...
def start_scada_stream():
    ...
    for msg in consumer:
        data = msg.value
        print(f"[SCADA Update] Transformer={data['TransformerID']} "
...
if __name__ == "__main__":
    ...
    if os.environ.get("ML4U_CI") == "1":
        print("ML4U_CI=1 detected: skipping Kafka SCADA stream consumption.")
    else:
