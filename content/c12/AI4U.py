...
def analyze_incidents_with_llm(logs):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        issues = [
            ...
            ("overheating", any("overheating" in l.lower() or "temperature" in l.lower() for l in logs)),
            ("vegetation", any("vegetation" in l.lower() for l in logs)),
            ("inspection", any("inspection" in l.lower() for l in logs)),
            ("cybersecurity", any("cyber" in l.lower() or "login" in l.lower() for l in logs)),
        ]
        ...
        summary = (
            ...
            "trim vegetation on affected feeders, and validate access controls at control center systems."
        )
        return summary
